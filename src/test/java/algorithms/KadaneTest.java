...
public class KadaneTest {
    ...
    @Test void single_element_negative(){
        var r = Kadane.maxSubarray(new int[]{-7});
        assertEquals(-7, r.sum); assertEquals(0, r.left); assertEquals(0, r.right);
    }

    @Test void all_negative(){
        var r = Kadane.maxSubarray(new int[]{-5,-2,-9});
        assertEquals(-2, r.sum); assertEquals(1, r.left); assertEquals(1, r.right);
    }

    @Test void mixed(){
        var r = Kadane.maxSubarray(new int[]{1,2,-5,4,7,-2});
        assertEquals(11, r.sum); assertEquals(3, r.left); assertEquals(4, r.right);
    }

    @Test void zeros(){
        var r = Kadane.maxSubarray(new int[]{0,0,0});
        assertEquals(0, r.sum); assertEquals(0, r.left); assertEquals(0, r.right);
    }
}
